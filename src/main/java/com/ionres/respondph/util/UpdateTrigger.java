...
public class UpdateTrigger {
    ...
    public boolean triggerCascadeUpdateWithDisaster(int beneficiaryId, int disasterId) {
        try {
            System.out.println("========== CASCADE UPDATE TRIGGERED (WITH DISASTER) ==========");
            System.out.println("Beneficiary ID: " + beneficiaryId);
            System.out.println("Disaster ID: " + disasterId);

            // Step 1: Update household score for this disaster
            HouseholdScoreCalculate householdCalc = new HouseholdScoreCalculate();
            boolean householdUpdated = householdCalc.calculateAndSaveHouseholdScore(beneficiaryId, disasterId);
            ...
            // ✅ CRITICAL FIX: Get aid types that have NULL disaster_id
            // These are the ones we'll UPDATE (not insert new records)
            List<AidTypeScoreRecord> nullDisasterRecords = getAidTypesWithNullDisaster(beneficiaryId);

            if (nullDisasterRecords.isEmpty()) {
                System.out.println("No existing NULL disaster records found for this beneficiary");
                System.out.println("This beneficiary may be brand new or already has disaster-specific records");
                System.out.println("========== CASCADE UPDATE COMPLETE ==========");
                return true;
            }

            System.out.println("Found " + nullDisasterRecords.size() + " NULL disaster record(s) to UPDATE");

            // Step 2: Recalculate and UPDATE each NULL record with the disaster_id
            // The calculation will automatically update the record with new scores based on disaster
            int successCount = 0;
            int failCount = 0;

            AidHouseholdScoreCalculate aidCalc = new AidHouseholdScoreCalculate();

            for (AidTypeScoreRecord record : nullDisasterRecords) {
                try {
                    // This will recalculate scores WITH the disaster and update the existing NULL record
                    boolean success = aidCalc.calculateAndSaveAidHouseholdScoreWithDisaster(
                            beneficiaryId,
                            record.aidTypeId,
                            record.adminId,
                    ...
                    if (success) {
                        successCount++;
                        System.out.println("✓ Updated NULL disaster record to disaster_id=" + disasterId +
                                " for aid type ID: " + record.aidTypeId);
                    } else {
                        failCount++;
                        System.err.println("✗ Failed to update record for aid type ID: " + record.aidTypeId);
                    }
                } catch (Exception e) {
                    failCount++;
                    System.err.println("✗ Error updating record for aid type ID " +
                            record.aidTypeId + ": " + e.getMessage());
                    e.printStackTrace();
                }
            }
            ...
            System.out.println("========== CASCADE UPDATE COMPLETE ==========");
            System.out.println("Successfully updated: " + successCount + " records");
            System.out.println("Failed: " + failCount);
    ...
    /**
     * ✅ NEW METHOD: Gets aid types that currently have NULL disaster_id
     * These will be UPDATED (not inserted) when disaster damage is added
     */
    private List<AidTypeScoreRecord> getAidTypesWithNullDisaster(int beneficiaryId) {
        List<AidTypeScoreRecord> records = new ArrayList<>();
        String sql = "SELECT aid_type_id, admin_id FROM aid_and_household_score " +
        ...
        try {
            ...
            System.out.println("Found " + records.size() + " NULL disaster record(s) for beneficiary " + beneficiaryId);

        } catch (SQLException e) {
            System.err.println("Error fetching NULL disaster records: " + e.getMessage());
            e.printStackTrace();
        } finally {
            try {
                if (conn != null && !conn.isClosed()) {
